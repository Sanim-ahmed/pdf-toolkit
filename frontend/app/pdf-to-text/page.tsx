...
export default function PdfToTextPage() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      setStats({ pages, chars, time });
    } catch (e) {
      setError(friendlyError(e instanceof Error ? e.message : "Extraction failed"));
    } finally {
      ...
      <main className="flex-1">
        <section className="relative overflow-hidden px-6 pb-24 pt-28 md:pt-36">
          ...
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h1 className="text-4xl font-extrabold tracking-tight md:text-5xl">PDF to<br /><span className="gradient-text">Text</span></h1>
            <p className="mx-auto mt-5 max-w-lg text-slate-400">Extract plain text content from any PDF document quickly and accurately.</p>
          </div>
          <div className="mx-auto mt-12 max-w-2xl">
            <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}
              onClick={() => inputRef.current?.click()} role="button" tabIndex={0}
              onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") inputRef.current?.click(); }}
              className={`relative cursor-pointer rounded-2xl border-2 border-dashed p-10 text-center transition-all duration-300 ${isDragging ? "border-cyan-500 bg-cyan-500/[0.08] scale-[1.02] shadow-2xl shadow-cyan-500/10" : "border-white/10 bg-white/[0.02] hover:border-cyan-500/40 hover:bg-cyan-500/[0.04]"}`}>
              ...
              <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-cyan-500/10">
                <svg className={`h-8 w-8 transition-colors ${isDragging ? "text-cyan-400" : "text-cyan-500/60"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
              </div>
              <p className="text-base font-medium text-white">{isDragging ? "Drop your PDF here" : "Drag & drop a PDF file here"}</p>
              <p className="mt-2 text-sm text-slate-500">or <span className="text-cyan-400 underline underline-offset-2">browse files</span></p>
              ...
              <div className="mt-6">
                ...
                {stats && (
                  <div className="mt-5 rounded-xl border border-cyan-500/20 bg-cyan-500/[0.04] p-4">
                    <div className="grid grid-cols-3 gap-3">
                      <div><p className="text-xs text-slate-500">Pages processed</p><p className="text-sm font-medium text-white">{stats.pages}</p></div>
                      <div><p className="text-xs text-slate-500">Characters extracted</p><p className="text-sm font-medium text-white">{stats.chars.toLocaleString()}</p></div>
                ...
                {downloadUrl ? (
                  ...
                  <button onClick={handleConvert} disabled={!file || isConverting} className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-cyan-600 to-teal-600 px-6 py-3.5 text-sm font-semibold text-white shadow-lg shadow-cyan-500/25 transition-all hover:-translate-y-0.5 hover:shadow-xl hover:shadow-cyan-500/30 disabled:pointer-events-none disabled:opacity-40">
                    {isConverting ? (
                      <><svg className="h-5 w-5 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>{progressText || "Extracting..."}</>
                    ) : (
                      <><svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h7" /></svg>Extract Text</>
                    )}
                  </button>
                )}
              </div>
