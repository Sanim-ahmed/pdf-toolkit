            ...
            {file && (
              <div className="mt-6">
                ...
                <div className="mt-5 space-y-3">
                  ...
                  {PRESETS.map((p) => (
                    <label key={p.value} className={`flex cursor-pointer items-center gap-4 rounded-xl border px-4 py-3 transition-all ${preset === p.value ? "border-emerald-500/40 bg-emerald-500/[0.06]" : "border-white/10 bg-white/[0.02] hover:border-white/20"}`}>
                      <input type="radio" name="preset" value={p.value} checked={preset === p.value} onChange={(e) => setPreset(e.target.value)} className="h-4 w-4 accent-emerald-500" />
                ...
                {downloadUrl ? (
                  ...
                  <button onClick={handleCompress} disabled={!file || isCompressing} className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-600 to-green-600 px-6 py-3.5 text-sm font-semibold text-white shadow-lg shadow-emerald-500/25 transition-all hover:-translate-y-0.5 hover:shadow-xl hover:shadow-emerald-500/30 disabled:pointer-events-none disabled:opacity-40">
                    {isCompressing ? (
                      <><svg className="h-5 w-5 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>{progressText || "Compressing..."}</>
                    ) : (
