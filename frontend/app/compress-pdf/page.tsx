...
export default function CompressPdfPage() {
  ...
  const handleCompress = useCallback(async () => {
    ...
    try {
      ...
      const time = parseFloat(res.headers.get("X-Compression-Time") || "0");
      setStats({ originalSize, compressedSize, ratio, time, compressionApplied });
    } catch (e) {
      setError(friendlyError(e instanceof Error ? e.message : "Compression failed"));
    } finally {
      ...
      <main className="flex-1">
        <section className="relative overflow-hidden px-6 pb-24 pt-28 md:pt-36">
          ...
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h1 className="text-4xl font-extrabold tracking-tight md:text-5xl">Compress<br /><span className="gradient-text">PDF</span></h1>
            <p className="mx-auto mt-5 max-w-lg text-slate-400">Reduce PDF file size while maintaining quality for easy sharing.</p>
          </div>
          <div className="mx-auto mt-12 max-w-2xl">
            <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}
              onClick={() => inputRef.current?.click()} role="button" tabIndex={0}
              onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") inputRef.current?.click(); }}
              className={`relative cursor-pointer rounded-2xl border-2 border-dashed p-10 text-center transition-all duration-300 ${isDragging ? "border-emerald-500 bg-emerald-500/[0.08] scale-[1.02] shadow-2xl shadow-emerald-500/10" : "border-white/10 bg-white/[0.02] hover:border-emerald-500/40 hover:bg-emerald-500/[0.04]"}`}>
              <input ref={inputRef} type="file" accept=".pdf" onChange={handleFileSelect} className="hidden" />
              <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-emerald-500/10">
                <svg className={`h-8 w-8 transition-colors ${isDragging ? "text-emerald-400" : "text-emerald-500/60"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
              </div>
              <p className="text-base font-medium text-white">{isDragging ? "Drop your PDF here" : "Drag & drop a PDF file here"}</p>
              <p className="mt-2 text-sm text-slate-500">or <span className="text-emerald-400 underline underline-offset-2">browse files</span></p>
            ...
            {file && (
              <div className="mt-6">
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <button onClick={removeFile} disabled={isCompressing} className="text-xs font-medium text-slate-500 transition-colors hover:text-red-400 disabled:pointer-events-none disabled:opacity-40">Clear all</button>
                </div>
                <ul className="space-y-2" role="list">
                  <li className="glass-card flex items-center gap-4 rounded-xl px-4 py-3 transition-all duration-200 hover:border-emerald-500/20">
                    ...
                    <button onClick={removeFile} disabled={isCompressing} className="ml-1 rounded-lg p-1.5 text-slate-500 transition-colors hover:bg-red-500/10 hover:text-red-400 disabled:pointer-events-none disabled:opacity-40" aria-label={`Remove ${file.name}`}>
                ...
                <div className="mt-5 space-y-3">
                  ...
                  {PRESETS.map((p) => (
                    <label key={p.value} className={`flex cursor-pointer items-center gap-4 rounded-xl border px-4 py-3 transition-all ${isCompressing ? "opacity-40 pointer-events-none" : ""} ${preset === p.value ? "border-emerald-500/40 bg-emerald-500/[0.06]" : "border-white/10 bg-white/[0.02] hover:border-white/20"}`}>
                      <input type="radio" name="preset" value={p.value} checked={preset === p.value} onChange={(e) => setPreset(e.target.value)} disabled={isCompressing} className="h-4 w-4 accent-emerald-500" />
                      <div><p className="text-sm font-medium text-white">{p.label}</p><p className="text-xs text-slate-500">{p.desc}</p></div>
                    </label>
                  ))}
                </div>
                {isCompressing && <LoadingOverlay accent="emerald" />}
                {error && <ErrorMessage error={error} />}
                {success && <SuccessMessage />}
                {stats && (
                  <div className="mt-5 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.04] p-4">
                    {!stats.compressionApplied && (
                      <div className="mb-3 rounded-lg border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-xs text-amber-400">This file could not be compressed further — the original is returned.</div>
                    )}
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div><p className="text-xs text-slate-500">Compressed Size</p><p className="text-sm font-medium text-white">{formatSize(stats.compressedSize)}</p></div>
                      <div><p className="text-xs text-slate-500">Reduction</p><p className="text-sm font-medium text-emerald-400">{stats.ratio.toFixed(1)}%</p></div>
                ...
                {downloadUrl ? (
                  ...
                  <button onClick={handleCompress} disabled={!file || isCompressing} className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-600 to-green-600 px-6 py-3.5 text-sm font-semibold text-white shadow-lg shadow-emerald-500/25 transition-all hover:-translate-y-0.5 hover:shadow-xl hover:shadow-emerald-500/30 disabled:pointer-events-none disabled:opacity-40">
                    {isCompressing ? (
                      "Processing..."
                    ) : (
                      <><svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 14l-7 7m0 0l-7-7m7 7V3" /></svg>Compress PDF</>
                    )}
                  </button>
                )}
              </div>
